...
const HomeSidebar = observer((props: Props) => {
  ...
  useEffect(() => {
    if (userStore.state.currentUser) {
      userStore.fetchUserStats(userStore.state.currentUser);
    }
  ...
  return (
    <aside className={cn("relative w-full h-full overflow-auto flex flex-col justify-start items-start custom-scrollbar", props.className)}>
      <SearchBar />
